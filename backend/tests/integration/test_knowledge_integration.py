...
class TestKnowledgeExtraction:
    ...
    async def test_discover_knowledge(self, client, test_agent_id):
        """测试从讨论内容提取知识"""
        response = await client.post("/api/knowledge/discover",
            params={
...
class TestKnowledgeRetrieval:
    ...
    async def test_search_knowledge(self, client):
        """测试搜索知识"""
        await client.post("/api/knowledge/discover",
            params={
    ...
    async def test_get_knowledge_detail(self, client):
        """测试获取知识详情"""
        discover_response = await client.post("/api/knowledge/discover",
            params={
        ...
        if knowledge_ids:
            detail_response = await client.get(f"/api/knowledge/{knowledge_ids[0]}")
    ...
    async def test_get_nonexistent_knowledge(self, client):
        """测试获取不存在的知识"""
        response = await client.get("/api/knowledge/nonexistent_id_12345")
...
class TestKnowledgeUsage:
    ...
    async def test_mark_knowledge_usage(self, client):
        ...
        if knowledge_ids:
            usage_response = await client.post(f"/api/knowledge/{knowledge_ids[0]}/use",
                params={"success": True}
            )
...
class TestPatternDiscovery:
    ...
    async def test_get_patterns(self, client):
        """测试获取所有模式"""
        response = await client.get("/api/knowledge/patterns")
        assert response.status_code == 200
        result = response.json()
...
class TestSkillGeneration:
    ...
    async def test_generate_skills(self, client, test_agent_id):
        """测试从成功案例生成技能"""
        await client.post("/api/knowledge/success-case",
        ...
        response = await client.post(f"/api/knowledge/skills/generate",
            params={"agent_id": test_agent_id}
        )
        assert response.status_code == 200
        result = response.json()
    ...
    async def test_get_skills(self, client):
        """测试获取所有技能"""
        response = await client.get("/api/knowledge/skills")
        assert response.status_code == 200
        result = response.json()
        assert "skills" in result
        assert "count" in result
...
class TestKnowledgeEvolution:
    ...
    async def test_knowledge_update_on_usage(self, client):
        ...
        if knowledge_ids:
            ...
            first_use = await client.post(f"/api/knowledge/{knowledge_id}/use", params={"success": True})
            first_stats = first_use.json()
            initial_count = first_stats.get("usage_count", 1)

            second_use = await client.post(f"/api/knowledge/{knowledge_id}/use", params={"success": True})
    ...
    async def test_confidence_adjustment(self, client):
        ...
        if knowledge_ids:
            knowledge_id = knowledge_ids[0]
            detail_before = await client.get(f"/api/knowledge/{knowledge_id}")

            for _ in range(3):
                await client.post(f"/api/knowledge/{knowledge_id}/use", params={"success": True})

            detail_after = await client.get(f"/api/knowledge/{knowledge_id}")
...
class TestKnowledgeWithMemory:
    ...
    async def test_knowledge_and_memory_integration(self, client, test_agent_id):
        """测试知识与记忆集成"""
        memory_response = await client.post("/api/memories/",
            json={
                "agent_id": test_agent_id,
                "content": "学习新技术栈的重要经验",
                "level": "long_term",
        ...
        knowledge_response = await client.post("/api/knowledge/discover",
            params={
        ...
        retrieved_memories = await client.get(f"/api/memories/agent/{test_agent_id}")
        assert retrieved_memories.status_code == 200

        knowledge_stats = await client.get("/api/knowledge/stats")
...
class TestKnowledgeErrorHandling:
    ...
    async def test_use_nonexistent_knowledge(self, client):
        """测试使用不存在的知识"""
        response = await client.post("/api/knowledge/nonexistent_id_12345/use",
            params={"success": True}
        )
        assert response.status_code == 404
